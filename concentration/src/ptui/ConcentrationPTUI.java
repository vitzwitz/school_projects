...
public class ConcentrationPTUI implements Observer {
     ...
     */
    private void displayBoard(int n, int up, ArrayList<Card> faces, boolean cheat) {
    ...
    public void update(Observable o, Object arg) {
        ...
        displayBoard(this.model.getMoveCount(),this.model.howManyCardsUp(),
                arg == null ? this.model.getCards() : this.model.getCheat(),false);

        // display a win if all cards are face up (not cheating)
        if (0 == this.model.getCards().stream().filter(
