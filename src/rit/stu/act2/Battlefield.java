...
public class Battlefield {
    ...
    private void printStatistics() {
        ...
        // TODO
        System.out.println("Statistics: " + this.enemyBase.getNumHostages() + " hostage/s remain, " +
    ...
    private void battle() {
        ...
        while (this.enemyBase.getNumHostages() > 0 && this.bunker.hasSoldiers()) {
            ...
            if (hostage != null) {
                System.out.println(hostage.toString() + " rescued from enemy base by soldier "
                        + soldier.toString());
                ...
                if (roll > this.predator.CHANCE_TO_BEAT_SOLDIER) {
                    soldier.victory(this.predator);
                    this.predator.defeat(soldier);
                    this.chopper.boardPassengers(hostage);
                    this.bunker.fortifySoldiers(soldier);
                } else {
                    this.predator.victory(soldier);
                    soldier.defeat(this.predator);
                    roll = Battlefield.nextInt(1, 100);
                    System.out.println(hostage.toString() + " encounter the predator who rolls a " + roll);
                    if (roll <= this.predator.CHANCE_TO_BEAT_HOSTAGES) {
                        this.predator.victory(hostage);
                        hostage.defeat(this.predator);
                    } else {
                        hostage.victory(this.predator);
                        this.predator.defeat(hostage);
                        this.chopper.boardPassengers(hostage);
                    }
                }
            }
            if (this.enemyBase.getNumHostages() == 0 && this.bunker.hasSoldiers()) {
                int temp = this.bunker.getNumSoldiers();
                for (int i = 0; i < temp; i++) {
                    Soldier Remainsoldier = this.bunker.deployNextSoldier();
                    this.chopper.boardPassengers(Remainsoldier);
                }
            }
        }
