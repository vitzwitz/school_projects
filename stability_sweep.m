...
%% Open document
fileID = fopen('stablity_control.txt','w');
...
for k=0:0.005:0.3 
    ...
    soln=solve(x.*(-x.^2 + (a+1).*x -(k+a))-y, e*(b*x-y)-k*y,[x y]);
        ...
        if(abs(imag(soln.x(j))<1e-10)&&...
           abs(imag(soln.y(j))<1e-10))
            ...
            if((real(myeigs(1)))<0)...
                ...
                %% Create document
                 fps = [xx yy k*ones(size(xx))];
                 
                 formatSpec = 'Fixed point at (%f,%f) with k = %.3f is stable \n';
                ...
                % plot with open circle
                 fpu = [xx yy k*ones(size(xx))];
                 formatSpec1 = 'Fixed point at (%f,%f) with k = %.3f is unstable \n';
...
title({'Bifurcation Plot for FitzHugh','Nagumo Model w/ Control Feedback'})